...
def create_model(**kwargs):
    from models.default_torch_newIAFF_with_doubleattn_weightadd_flashattn import MyParticleNetwork
    # from models.default_torch_newIAFF_with_doubleattn_weightadd import MyParticleNetwork
    # from models.default_torch_baseline import MyParticleNetwork
...
def main():
    ...
    parser.add_argument(  "--cfg",
                        type=str,
                        default=r".../scripts/default_69R.yaml",
        ...
        if (trainer.current_step) % (1.0 * _k) == 0:
            for k, v in evaluate_torch(model,
                ...
                if(k == "err_n1" and v < min_err):
                    min_err = v
                    torch.save({'model': model.state_dict()}, outdir+str(step.item())+'_model_weights_best.pt')
                    print("=================update best model: err_n1=" + str(v) + "===============")

    # 第1000次迭代eval一次

    torch.save({'model': model.state_dict()}, outdir+'/model_weights.pt') #所有batch遍历一遍记录下当前model
